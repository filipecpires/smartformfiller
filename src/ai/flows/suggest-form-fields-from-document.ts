...
const SuggestedFieldSchema = z.object({
  ...
  type: z.enum(['text', 'date', 'number', 'email', 'dropdown_candidate']).describe('The most likely type for this field (e.g., text, date). Use "dropdown_candidate" if it seems like a field that would benefit from predefined options.'),
  ...
  // possibleOptions: z.array(z.string()).optional().describe('If type is dropdown_candidate, suggest 3-5 distinct example values from the document that could serve as options, if discernible.')
});
export type SuggestedField = z.infer<typeof SuggestedFieldSchema>;

const SuggestFormFieldsOutputSchema = z.object({
  suggestedFields: z.array(SuggestedFieldSchema).describe('A list of suggested form fields with their labels, types, and example values.'),
});
...
Instructions:
1.  Thoroughly analyze the document to identify distinct pieces of information that would typically be captured in a form (e.g., names, dates, addresses, identification numbers, amounts, specific terms, clauses, etc.).
2.  For each identified piece of information, suggest a form field:
    a.  **Label**: Create a clear, concise, and human-readable label for the field. The label should accurately represent the information it captures (e.g., "Nome do Contratante", "Data de Início", "Valor do Aluguel", "CPF", "Endereço Completo").
    b.  **Type**: Determine the most appropriate data type. Choose from: 'text', 'date', 'number', 'email'. If you identify a field that seems to have a limited set of recurring, distinct values within the document, suggest 'dropdown_candidate' as the type.
    c.  **Example Value**: If you can extract a clear and unambiguous example of this information directly from the document, provide it. Otherwise, omit this. For dates, try to provide them in YYYY-MM-DD format if possible, or as they appear.
...
5.  Return the suggestions as an array of objects, where each object contains 'label', 'type', and optionally 'exampleValue'.
...
- { label: "Tipo de Imóvel", type: "dropdown_candidate", exampleValue: "Apartamento" }
